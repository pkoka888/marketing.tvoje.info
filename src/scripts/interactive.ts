...
export function initMobileMenu(): void {
  ...
  mobileMenuButton.addEventListener('click', () => {
    // Toggle visibility with animation - check current state
    const wasExpanded = mobileMenuButton.getAttribute('aria-expanded') === 'true';
    void wasExpanded; // State check for potential future use
...
function createScrollToTopButton(): void {
  ...
  button.className =
    'fixed bottom-8 right-8 p-3 rounded-full bg-primary-600 text-white shadow-lg transition-all duration-300 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:focus:ring-offset-dark-900 hidden z-50';
