...
## 5. Tool Call Protocol (STRICT)

To avoid "missing parameter" errors, ALWAYS include all required arguments.

### Golden Examples:

- **execute_command**: `{"command": "npm run test"}`
- **read_file**: `{"path": "c:/path/to/file.ts"}`
- **write_to_file**: `{"path": "c:/path/to/file.ts", "content": "..."}`
- **web_search**: `{"query": "marketing trends 2026"}`

**If a tool fails due to missing parameters, DO NOT retry without adding the missing values.**
